...
function MealItem({ meal }) {
	const defaultCardImg = "/images/default-item.png"
	...
	return (
		<div className="relative mb-10 md:mb-0 ">
			<Card cu shadow={0} padding={0} radius={0}>
				<Card.Section>
					<Link href={`/meal/${meal.id}`}>
						<Image src={meal.image || defaultCardImg} height={160} alt="Norway" />
					</Link>
				</Card.Section>

				<Group className="cursor-pointer" position="apart" mt="md">
					<Link href={`/meal/${meal.id}`}>
						<Text size={20} weight={600} color="#1F2937" transform="capitalize">{meal.name}</Text>
					</Link>
				</Group>
				{renderMealTag()}
				<Text size={14} weight={400} color="#1F2937" lineClamp={3} transform="capitalize">
					{meal.description}
				</Text>
				<Group position="apart" mb="xs" className="w-full">
					<div className="flex items-center justify-between w-full">
						...
						<div>
							<Rating defaultValue={meal.rating} readOnly color="blue" size="sm" />
						</div>
					</div>
				</Group>
				<div>
